...
public class RegisterController extends ControllerHelper {
    ...
    /** 
     * Description: 通过邮箱给用户发送验证码<br>
     * Create by: jing_liu@asdc.com.cn <br>
     * Create Time: 2016年6月17日<br>
     * @throws ServerException 
     */   
    public void sendCode() throws ServerException{
    	String emailRegister=workDTO.get("emailRegister").toString().replace("\"", "");
    	EmailConfig config = EmailConfig.getInstance();
        Email em = new Email();
        em.setMailReceiver(emailRegister);
        em.setSubject("你好，请您确认");
        String verifyCode=getVerifyCode();
        em.setContent("您的验证码是:"+verifyCode);
        log.debug("邮件内容{}",em.getContent());
        sessionDTO.put("verifyCode", verifyCode.toString());
