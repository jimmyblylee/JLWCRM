...
public class UserMgtServiceImpl implements UserMgtService {
     ...
     * @param id SysUser, SysUserPhoto, SysUserPhoto
	...
	public int getSysUsersByDeptAndAccountAndNameCount(SysUser sysUser) throws ServiceException {
		...
		int userCount = authService.queryTotle("sys.hql.querySysUsersByDeptAndAccountAndNameCount", Parameter.toList("deptId", sysUser.getDeptId(), "account", name, "name", name,"isEnabled",sysUser.getIsEnabled()));
		return userCount;
	}
	...
	public SysUser updateSysUserInfo(SysUser sysUser) throws ServiceException {
		...
		//更新用户基本信息
		SysUser updateSysUser = updateSysUser(sysUser);
	...
	public SysUser getUserById(Integer userId) throws ServiceException{
		SysUser sysUser=(SysUser) sysCommonDao.getSingleResultByNamedQuery("sys.hql.getUserById", Parameter.toList("id",userId));
	    return sysUser;
	}
	...
	public void repeatSysUser(List<Integer> ids) throws ServiceException {
        ...
        for (Integer id : ids) {
        	user = getSysUserById(id);
            user.setIsEnabled(true);
            dept = user.getDept();
            if(dept.getIsEnabled()==false){
