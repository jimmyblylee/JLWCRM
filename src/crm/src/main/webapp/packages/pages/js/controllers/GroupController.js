 ...
 * 修改用户组权限
 */
function updateGroupRole($scope, $filter, $http, $q) {
	var data = mergeReauestData('FuncController', 'queryAllSysFuncGroup', "");
	...
	result.then(function(value) {
				...
				var FuncGroupArray = value.result;
				console.log("数据列表：--字符串形式" + FuncGroupArray);
				// console.log("数据列表：--json形式"+JSON.parse(GroupArray));

				$scope.items = FuncGroupArray;
	})
}
 ...
 * 查找所有的用户分组
 */
function GrouptableCtrl($scope, $filter, $http, $q) {
	$scope.filteredItems = [];
	$scope.groupedItems = [];
	$scope.currentPage = 1;
	 ...
	 */
	var data = mergeReauestData('GroupController', 'queryAllSysGroup', "");
	// 发送get请求
	var result = sendPost($http, data, $q);
			result
					.then(function(value) {
						...
						value = JSON.parse(value);
						// console.log("value"+value.result);
						var GroupArray = value.result;
						//console.log("数据列表：--字符串形式" + GroupArray);
						// console.log("数据列表：--json形式"+JSON.parse(GroupArray));

						$scope.items = GroupArray;
						...
						$scope.sort_by = function(newSortingOrder) {
							...
							if ($scope.reverse) {
								$('th.' + newSortingOrder + ' a').removeClass()
			...
			$scope.update =function(a){
				$scope.updateGroup=$scope.items[a]
			}
}
