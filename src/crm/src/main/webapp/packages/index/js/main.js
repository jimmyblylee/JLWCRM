...
//切换日历的语言
(function datepicker_i18n() {
    var getCookieLang = function(name) {
    ...
    var localeI18n = getCookieLang("languageChoice");
    if (localeI18n == "" || localeI18n == null || localeI18n == undefined) {
        localeI18n = "zh-cn"
    }
...
MetronicApp.factory('httpInterceptor', ['$q', '$injector', '$rootScope', '$window', function ($q, $injector, $rootScope, $window) {
    return {
...
MetronicApp.controller('AppController', ['$scope', '$rootScope', '$translate', '$window', '$state', '$http', '$q', '$interval', '$modal', function ($scope, $rootScope, $translate, $window, $state, $http, $q, $interval, $modal) {
    $rootScope.sysTime = new Date();
    $rootScope.getSysTimedataFun = function () {
        var getSysTimedata = mergeReauestData('GlobalListController',
            'getGlobalSysTime', "");
        var getSysTime = sendPost($http, getSysTimedata, $q);
        getSysTime.then(function (success) {
            var sysTime = StrParesJSON(success);
        ...
        if (userId != undefined && userId != null) {
            ...
            tokenResult.then(function (restokenset) {
                if (restokenset.indexOf("loginTimeOut") != -1) {
                    ...
                    $rootScope.accountIshideOrShow = funcCode.indexOf("account") != -1;

                    $rootScope.iconIshideOrShow = funcCode.indexOf("icon") != -1;
...
MetronicApp.controller('ThemePanelController', ['$scope', '$http', '$q', '$rootScope', function ($scope, $http, $q, $rootScope) {
    ...
    $scope.getGlobalset = function (globalCode, globalVal) {
        ...
        getGlobalResult.then(function (success) {
            ...
            if (globalVal != "setColorNewData") {
                $scope.themeData[globalVal.substring(3)] = $scope.globalData[globalVal].variableDescribe
            }
    ...
    $scope.setTheme = function (newTheme, themeName) {
        var positionDir;
        if (newTheme == 'top') {
            positionDir = $scope.globalData.SidebarPostionTop.variableDescribe;
            $rootScope.settings.tplPath = positionDir;
            $rootScope.settings.layout.pageFullWidth = true;
        } else if (newTheme == 'left') {
            positionDir = $scope.globalData.SidebarPostionLeft.variableDescribe;
            $rootScope.settings.tplPath = positionDir;
            $rootScope.settings.layout.pageFullWidth = false;
        } else if (newTheme == 'right') {
            positionDir = $scope.globalData.SidebarPostionRight.variableDescribe;
            $rootScope.settings.tplPath = positionDir;
            $rootScope.settings.layout.pageFullWidth = false;
        }
