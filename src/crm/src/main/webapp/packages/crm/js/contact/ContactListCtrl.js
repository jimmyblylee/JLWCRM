 ...
 */
function ContactListCtrl($scope, $http, $q, $listService) {
    ...
    $scope.prepareToAdd = function() {
        var modalScope = $("div[ng-controller='ContactModifyCtrl']").scope();
        modalScope.contact = {
        ...
        modalScope.title = "增加联系人";
        modalScope.method = "create";
    };

    $scope.prepareToUpdate = function(item) {
        var modalScope = $("div[ng-controller='ContactModifyCtrl']").scope();
        modalScope.contact = item;
        modalScope.title = "更新联系人";
